...
import {createArticleCategory, fetchArticleCategories} from "../../../features/categories/articleCategoryThunks.ts";
...
const CreateArticleCategory: React.FC = () => {
    ...
    const handleSubmit = async (values: typeof initialValues) => {
        ...
        try {
            ...
            await dispatch(createArticleCategory({ ...values, parentId: parentIdInt })).unwrap();
    ...
    const sortedArticleCategories = [...categories].sort((a, b) => {
        const dateA = new Date(a.createdAt);
        const dateB = new Date(b.createdAt);
        return dateB.getTime() - dateA.getTime(); // descending order
    });

    const formattedCategories = sortedArticleCategories.map((category) => ({
        value: category.id,
